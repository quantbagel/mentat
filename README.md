...
This repo also contains **Mentat**, a public reimplementation effort for
compiled computation inside transformers. Current scope is standalone compiled
kernels and experiment harnesses, not integration into nanochat's main model
path. See [MENTAT.md](MENTAT.md). For the first synthetic training bootstrap,
use `bash runs/mentat_stack.sh`.
